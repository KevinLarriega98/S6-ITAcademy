...
import { WEB_CONFIGURATION_UNIT_PRICE } from "../lib/constants/pricingConstants"
...
export const buildDiscountedPriceMap = (services: ServiceOption[], discountMultiplier: number) =>
  new Map(
    services.map((service) => [service.id, roundCurrency(service.price * discountMultiplier)] as const),
  )
